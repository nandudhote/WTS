...
void myWIFI::MQTT_Pull(char* topic, byte* payload, unsigned int length) {
  ...
  if (mqttDecodedMsg.indexOneData == "wiFiCredentials") {
    ...
    alertMsg = String(firstThresholdTempForAlert) + ":" + String(buzzerBeepFlag) + ":" + String(interval) + ":" + String(secondThresholdTempForAlert);
    ...
    iiDrivers.writeOneByteInEEPROM(maxThresholdTempForAlertEEPROMAdd, secondThresholdTempForAlert);
