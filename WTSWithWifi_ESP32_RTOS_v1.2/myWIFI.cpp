...
void myWIFI::MQTT_Pull(char* topic, byte* payload, unsigned int length) {
  ...
  if (mqttDecodedMsg.indexOneData == "wiFiCredentials") {
    ...
    alertMsg = SSID + ":" + PASSWORD + ":" + String(activeSensor) + ":" + String(buzzerBeepFlag) + ":" + String(interval) + ":" + String(firstThresholdTempForAlert) + ":" + String(secondThresholdTempForAlert);
  } else if (mqttDecodedMsg.indexOneData == "serverStatus") {
    alertMsg = String(ServerMQTT) + ":" + String(MqttPort);
  ...
  } else if (mqttDecodedMsg.indexOneData == "MACAddress") {
    char buf[18];
    sprintf(buf, "%02X:%02X:%02X:%02X:%02X:%02X", MAC[0], MAC[1], MAC[2], MAC[3], MAC[4], MAC[5]);
    alertMsg = String(buf);
  } else if (mqttDecodedMsg.indexOneData == "MqttServer") {
    char buf[18];
    alertMsg = String(ServerMQTT) + ":" + String(MqttPort);
  } else {
    delay(1);
